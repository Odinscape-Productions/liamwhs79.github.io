...
The previous build used optional Jina Reader headers such as browser/no-cache modes anonymously.
Those enhanced Reader options can be rejected with HTTP 401.

v8 first uses a plain anonymous Reader GET with no privileged options, then tries public CORS mirrors.
Tracker errors are shown separately from the official PlayStation Share Profile connection.
