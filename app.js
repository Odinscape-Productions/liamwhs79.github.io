...
function renderHome(){
  ...
  $("#home-sync-button").textContent=synced?"REFRESH PSN PROFILE":"SYNC PSN PROFILE";
...
function openImportModal(){
  ...
  openModal(`<span class="eyebrow">PUBLIC PSN PROFILE IMPORT</span><h2>Build my real game library</h2><p class="modal-intro">PulseStation now uses Exophase's public profile plus its public JSON game feed where available. PSNProfiles and TrueTrophies are secondary stat checks. No Sony login is used.</p><form id="import-form"><div class="field"><label>PSN Online ID</label><input name="onlineId" value="${esc(id)}" maxlength="32" required></div><details class="advanced-import"><summary>Advanced fallback</summary><div class="field" style="margin-top:10px"><label>Exophase username or profile URL (optional)</label><input name="exophaseHint" placeholder="Only use this if your Exophase name differs from your PSN ID"></div></details><div class="tracker-banner"><b>Important:</b> the curated titles in Discover are not treated as yours. My games is populated only from games actually returned by your tracker import or games you add manually.</div><div class="import-progress" id="import-progress"><div class="import-row"><span>Ready</span><span>Enter your PSN ID</span></div></div><div id="import-error" class="form-error"></div><div class="modal-actions"><button class="primary-button" id="import-submit">Fetch my public profile</button></div></form>`);

  $("#import-form").addEventListener("submit",async e=>{
    e.preventDefault();
    const fd=new FormData(e.currentTarget);
    const onlineId=String(fd.get("onlineId")||"").trim();
    const exophaseHint=String(fd.get("exophaseHint")||"").trim();
    const progress=$("#import-progress");
    progress.innerHTML="";
    $("#import-error").textContent="";
    $("#import-submit").disabled=true;
    $("#import-submit").textContent="Fetching real profile…";

    try{
      const result=await Importer.importProfile(onlineId,p=>{
      ...
      Store.mutate(db=>{
        ...
        if(!me) throw new Error("Local account could not be found.");

        me.psnOnlineId=onlineId;
        me.handle=onlineId;
        ...
        // Keep games the user manually added from Discover, replace only old tracker imports.
        ...
        const tracked=importedGames.map(x=>({
          ...
          lastPlayed:x.lastPlayed
            ? new Date(x.lastPlayed).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})
            : "Tracked"
        }));
        me.library=[...tracked,...manual];
      });

      Store.addActivity(state.me.id,"sync",`Imported ${importedGames.length} public PlayStation games for ${onlineId}`);
      reload();

      const unavailable=(result.failures||[]).length;
      progress.innerHTML=`<div class="import-result-card"><strong>${importedGames.length} games imported</strong><span>Level ${result.level||"—"} • ${result.platinum||0} platinum • ${Number(result.hours||0).toLocaleString()} tracked hours</span><small>${result.sources.length} public source${result.sources.length===1?"":"s"} responded${unavailable?` • ${unavailable} unavailable`:""}.</small></div>`;

      $("#import-submit").disabled=false;
      $("#import-submit").textContent="Open My games";
      $("#import-submit").type="button";
      $("#import-submit").onclick=()=>{
        closeModal();
        state.gameFilter="owned";
        $$("[data-game-filter]").forEach(b=>b.classList.toggle("active",b.dataset.gameFilter==="owned"));
        route("games");
        renderGames();
        toast(`${importedGames.length} tracked games added to My games.`);
      };
    }catch(err){
      $("#import-error").innerHTML=`<b>Nothing was imported.</b><br>${esc(err.message)}<br><br><span style="color:var(--muted)">This usually means the PSN ID is not indexed by a public tracker, its tracker/privacy settings prevent reading it, or the tracker temporarily blocked the public reader.</span>`;
      $("#import-submit").disabled=false;
      $("#import-submit").textContent="Try again";
    }
  });
}
